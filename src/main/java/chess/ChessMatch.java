...
public class ChessMatch {
    ...
    private Piece makeMove(Position source, Position target) {
        ...
        if (p instanceof King) {
            moveRookForCastling(source, target);
        }

        // special move: en passant
        if (p instanceof Pawn) {
            capturedPiece = performEnPassantCapture(p, source, target, capturedPiece);
        }
    ...
    private void undoMove(Position source, Position target, Piece capturedPiece) {
        ...
        // undo castling
        if (p instanceof King) {
            undoMoveRookForCastling(source, target);
        }

        // undo en passant
        if (p instanceof Pawn) {
            undoEnPassantCapture(p, target, capturedPiece);
        }
    }

    private void moveRookForCastling(Position source, Position target) {
    ...
    private void undoMoveRookForCastling(Position source, Position target) {
        if (target.getColumn() == source.getColumn() + 2) {
            Position sourceRook = new Position(source.getRow(), source.getColumn() + 3);
            Position targetRook = new Position(source.getRow(), source.getColumn() + 1);
    ...
    private Piece performEnPassantCapture(ChessPiece pawn, Position source, Position target, Piece capturedPiece) {
        if (source.getColumn() != target.getColumn() && capturedPiece == null) {
            Position pawnPosition = pawn.getColor() == Color.WHITE
    ...
    private void undoEnPassantCapture(ChessPiece pawn, Position target, Piece capturedPiece) {
        if (capturedPiece != null && capturedPiece == enPassantVulnerable) {
            ChessPiece capturedPawn = (ChessPiece) board.removePiece(target);

            Position originalPosition = pawn.getColor() == Color.WHITE
                    ? new Position(4, target.getColumn())
                    : new Position(3, target.getColumn());

            board.placePiece(capturedPawn, originalPosition);
        }
    }
    ...
    private ChessPiece king(Color color) {
        return piecesOnTheBoard.stream()
                ...
                .findFirst()
                .orElseThrow(() ->
                        new IllegalStateException("На доске нет короля цвета " + color));
    }
